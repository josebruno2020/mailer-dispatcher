<div>
    ...
    <x-content>
        <form wire:submit.prevent="save">
            <div class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <x-input id="subject" type="text" class="mt-1 block w-full"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <textarea id="body" wire:model.defer="form.body"
                            ...
                            placeholder="Digite seu HTML aqui..." oninput="updatePreview()"></textarea>
                    ...
                    <div wire:ignore>
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <div class="flex space-x-2">
                                <button type="button" onclick="togglePreviewMode('desktop')" id="desktop-btn"
                                    class="px-2 py-1 text-xs bg-blue-500 text-white rounded">
                                    Desktop
                                </button>
                                <button type="button" onclick="togglePreviewMode('mobile')" id="mobile-btn"
            ...
            <div class="my-5">
                <x-label for="webhook_url" value="{{ __('URL webhook') }}" />
                <x-input id="webhook_url" type="text" class="mt-1 block w-full"
                    wire:model.defer="form.webhook_url" />
                <x-input-error for="webhook_url" class="mt-2" :messages="$errors->get('form.webhook_url')" />
            </div>
            <div class="grid grid-cols-2 gap-6">
                <div>
                    <x-label for="webhook_headers" value="{{ __('Headers webhook') }}" />
                    <x-input  id="webhook_headers" class="mt-1 block w-full" wire:model.defer="form.webhook_headers" />
                    <x-input-error for="webhook_headers" class="mt-2" :messages="$errors->get('form.webhook_headers')" />
                </div>
            </div>
            ...
            <div class="mt-6">
                ...
                <div class="">
                    @foreach ($parameters as $parameter)
                        <div
                            class="mb-1 bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 p-1 rounded text-sm">
                            {{ $parameter }}
                        </div>
                    @endforeach
                </div>
            ...
            <div class="flex items-center justify-end mt-6">
                <x-secondary-button 
                x-on:click.prevent="$dispatch('open-modal', 'generate-email')"
                 class="mx-3">
                    {{ __('Generate Email') }}
                </x-secondary-button>
    ...
    @include('livewire.modals.email-generate', ['showGenerateModal' => 'isGenerateModal'])
            ...
            try {
                ...
                if (htmlContent.trim() === '') {
                    previewDiv.innerHTML =
                        '<p class="text-gray-500 italic">Digite seu HTML no editor para ver o preview...</p>';
                } else {
        ...
        const insertLink = () => {
            try {
                ...
                if (url) {
                    ...
                    textarea.value = textarea.value.substring(0, start) + insertion + textarea.value.substring(textarea
                        .selectionEnd);
                    textarea.focus();
                    updatePreview();
